...
in
{
  wayland.windowManager.hyprland = {
    ...
    # Mod5 == AltGr
    extraConfig = ''
      bind = ALT MOD5, O, submap, obscontrol
      submap = obscontrol
      bind = ALT, F10, pass, class:^(com\.obsproject\.Studio)$
      bind = ALT, F9, pass, class:^(com\.obsproject\.Studio)$
      bind = ALT, M, pass, class:^(com\.obsproject\.Studio)$
      bind = , escape, submap, reset
      bind = ALT MOD5, O, submap, reset
      submap = reset

      bind = ALT MOD5, D, submap, discordcontrol
      submap = discordcontrol
      bind = ALT, M, pass, class:^(legcord)$
      bind = , escape, submap, reset
      bind = ALT MOD5, D, submap, reset
      submap = reset
    '';
  };
}
